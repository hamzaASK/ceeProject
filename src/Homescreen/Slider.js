...
class CarouselPage extends Component
{
    ...
    convertBooks ()
    {
        ...
        console.log( this.state.booksConvert )
    }

    render ()
    {
        let x = this.props.lang === 'fr' ? 0 : 1
        let array = this.state.booksConvert === null || this.state.booksConvert === 'undefined'
            ? lang[ x ].welcome : Array.from( this.state.booksConvert[ x ] )
        console.log( array )
                        ...
                        array.map( ( x ) =>
                        {
                            return (
                                <MDBCarouselItem itemId={ x.id } key={ x.id }>
                                    <MDBView >
                                        <img className="d-block w-100" src={ "images/home/" + x.picture } alt="slide show" style={ { borderRadius: 10 } } />
                                    </MDBView>
