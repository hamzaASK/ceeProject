...
/***************************************/
deleteBook(id) {
  axios.delete(`${URL_Host1}/api/deleteflore/` + id).then((response) => {
    this._refreshBooks();
  });
}
/***************************************/
_refreshBooks() {
  axios.get(`${URL_Host1}/api/flore`).then((response) => {
    this.setState({
      books: response.data
        })
...
Upload = async () => {
    ...
    /****         Create Request          ****/
    let req = new Request('http://localhost:8080/ceedisplay/ceedisplay/src/Adminspace/php/uploadFlore.php',{method: 'POST',headers: h,mode: 'no-cors',body: fd});
    ...
    axios.put(`${URL_Host1}/api/updateflore/` + this.state.editBookData.id, { infoeng,infoFR,nomFR,nomAR,infoAR,nomeng,statuts_conservation,image })
      .then((response) => {
        this.setState({
          imageSrc: null,
          crop: { x: 0, y: 0 },
        ...
        <Table2 className={classes.table}>
          ...
          <TableBody>{this.state.books.map(row => (
            <TableRow className={classes.row} key={row.id}>
              ...
              <img alt='img' src={row.image==='WithoutImage' || !row.image ? 'images/flora/1.png' : "images/flora/"+row.image} style={{width:"140px",height:"70px",borderRadius:'25px'}}/>
              </CustomTableCell>
              <CustomTableCell align="center">
...
const A10 = withStyles(styles)(App);
export default A10
