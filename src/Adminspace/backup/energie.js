...
class App extends Component {
  ...
  updateBook() {
    ...
    axios.put('http://localhost:8001/api/updateenergie/' + this.state.editBookData.id, {
      varvalue, lname
    }).then((response) => {
      this._refreshBooks();
  ...
  _refreshBooks() {
    axios.get('http://localhost:8001/api/energie').then((response) => {
      this.setState({
        books: response.data
      })
    });
